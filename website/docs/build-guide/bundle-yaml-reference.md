...
3. `aether_ops.onramp_password` in the bundle spec — this field. Honoured
   when set, but should only be used in gitignored / scratch specs.
4. A random 24-character password generated by the installer and logged
   to stderr with an `IMPORTANT: record this password` banner — lowest.
