...
The `apt-repo/` tree is a real apt-archive layout. At install time the launcher moves it from the temp extract directory to `/var/lib/aether-ops/apt-repo/` (same persistence pattern as `/var/lib/aether-ops/aether-onramp/` and `/var/lib/aether-ops/helm-charts/`), drops a `sources.list` at `/etc/apt/sources.list.d/aether-bundle.list` pointing `file://` at it, and runs `apt-get install` for the spec's top-level packages — apt walks the closure (Depends/Conflicts/Breaks/Replaces) against the bundled metadata, picks an install plan, and dpkg's trigger system fires correctly because the transaction looks like any other apt transaction. The two apt-get calls during install are scoped via `-o Dir::Etc::SourceList=… -o Dir::Etc::SourceParts=/dev/null` so the host's image-baked archive.ubuntu.com entries (unreachable in airgap) don't poison the install. After bootstrap the persistent sources.list survives so the operator can `apt install <something else from the bundle>` without manually reconstructing the source. Bundle schema bumped to `2` when this layout took over from the previous `dpkg -i --force-*` path.
