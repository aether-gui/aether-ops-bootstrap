...
func TestPromote_EmitsSecurityArtifacts(t *testing.T) {
	...
	for _, want := range []string{
		...
		`filename: image-sboms.tar.gz`,
		`source: ../dist/image-sboms.tar.gz`,
		`filename: image-grypes.tar.gz`,
		`source: ../dist/image-grypes.tar.gz`,
...
func TestPromote_DemotesSecurityArtifacts(t *testing.T) {
	...
	// "first" entry has ten security_artifacts entries (4 on
	// bootstrap, 6 on bundle including the two image-* tarballs), so
	// we must supply ten matching SHAs.
	err := Promote(Options{
		YAMLPath:    yamlPath,
		SpecPath:    specPath,
		...
		PriorSecurity: []PriorSecuritySHA{
			...
			{ParentKind: "bundle", Filename: "image-sboms.tar.gz", SHA256: "image-sboms-sha"},
			{ParentKind: "bundle", Filename: "image-grypes.tar.gz", SHA256: "image-grypes-sha"},
	...
	for _, want := range []string{
		...
		`sha256: "image-sboms-sha"`,
		`sha256: "image-grypes-sha"`,
	...
	// The new release re-introduces every source: line, so we expect
	// exactly one occurrence (the new entry's), not two.
	for _, src := range []string{
		...
		`../dist/image-sboms.tar.gz`,
		`../dist/image-grypes.tar.gz`,
	} {
